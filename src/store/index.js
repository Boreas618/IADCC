...
import CaseRecord from "store/case_record";
...
class CATStore {
    ...
    @observable currentModule = 0;

    @observable currentFrameIndex = 0;

    @observable clickID = -1;

    @observable filteredTimestamp = [];

    @observable differentiated = false;

    @observable caseRecord = new CaseRecord();

    @observable selectedInfo = {metric: 'velocity', val: 0};
    ...
    @action navigateTo(module) {
        this.currentModule = module;
    }

    @action changeFrameIndex(index) {
        this.currentFrameIndex = index;
    }

    @action filterTimestamp(list) {
        this.filteredTimestamp = list;
    }

    @action changeClickID(id) {
        this.clickID = id;
    }

    @action selectMetric(metric, val) {
        console.log(metric);
        console.log(val);
        this.selectedInfo = {metric: metric, val: val};
    }
    ...
    handleOptionToggle(option) {
      ...
      if (!oldShowMonitor && this.options.showMonitor) {
        ...
        this.dimension.update();
         //-----------------2022.1.21
         if(this.options.showPlanningEva) {
          this.dimension.update();
        }
      }
