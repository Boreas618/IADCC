...
export default class Dimension {
    ...
    @action updateMainDimension() {
      ...
      console.log('main w/h',this.main.width, this.main.height);
    }

    @action updatePlanningDimension() {
      let widthOffset = 0;
      if (!OFFLINE_PLAYBACK) {
        widthOffset = this.isSmallScreen()
          ? MAIN_STYLE.SIDE_BAR_WIDTH_SMALL : MAIN_STYLE.SIDE_BAR_WIDTH_LARGE;
      }
      this.myview.width = (this.pane.width - widthOffset) *7/18;
      this.myview.height = this.pane.height * 0.5;
    }
    ...
    @action updateSceneDimension() {
      ...
      }  else if (this.options.showPlanningEva) {
    ...
    @action updateNavigationDimension() {
      ...
      if(this.options.showPlanningEva) {
        ...
        this.navigation.width = mainWidth * 0.23;
      } else {
